...
fn main() {
    ...
    let image = image::load(Cursor::new(&include_bytes!("../rainbow.png")[..]),
    ...
    let mut position = cgmath::Vector3::<f32>::new(0.0, 0.0, 0.0);
    ...
    support::start_loop(event_loop, move |events| {
        tenants.rent_mut(|(framebuffer, light_buffer, dt)| {
            // Animation
            position.x = now.elapsed().as_secs_f32() % 2.0;
            println!("{}", position.x);
            // now = Instant::now();
            ...
            let uniforms2 = uniform! {
                perspective_matrix: Into::<[[f32; 4]; 4]>::into(perspective_matrix),
                view_matrix: Into::<[[f32; 4]; 4]>::into(view_matrix),
                model_matrix: Into::<[[f32; 4]; 4]>::into(cgmath::Matrix4::from_translation(position)),
                tex: &opengl_texture
            };
            ...
            target.draw(&floor_vertex_buffer, &floor_index_buffer, &prepass_program, &uniforms2, &Default::default()).unwrap();
            ...
            for event in events {
                match event {
                    glutin::event::Event::WindowEvent { event, .. } => match event {
                        ...
                        glutin::event::WindowEvent::KeyboardInput { input, .. } => println!("{:?}", input.virtual_keycode),
                        _ => (),
                    },
                    _ => (),
                }
